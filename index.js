...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --bg: #07090f; --bg-alt: #0b0e17; --surface: #111623; --surface-hi: #161c2d;
    --border: #1d2438; --border-hi: #2a3352; --text: #f1f3f9; --text-mid: #b4bccd;
    --text-dim: #7683a0; --accent: #5b8eff; --ok: #3ddc97; --warn: #ffb454; --danger: #ff6b6b;
  ...
  .verdict { display: inline-block; padding: 4px 10px; border-radius: 6px; font-family: var(--mono); font-size: 0.72rem; font-weight: 600; text-transform: uppercase; }
  ...
  .reasons { color: var(--text-dim); font-size: 0.82rem; max-width: 340px; }
  ...
  .spinner { display: inline-block; width: 14px; height: 14px; border: 2px solid var(--border-hi); border-top-color: var(--accent); border-radius: 50%; animation: spin 0.7s linear infinite; }
    ...
    <div id="dashboard" ${token ? '' : 'style="display:none"'}>
      ...
      <div class="table-wrap">
        <div class="table-header">
          <div>Recent verifications</div>
          <button class="refresh-btn" id="refresh">Refresh</button>
        </div>
        <div id="table-body"><div class="empty"><span class="spinner"></span> Loading...</div></div>
      </div>
    </div>
  </div>
<script>
...
document.getElementById('token-input')?.addEventListener('keydown', (e) => { if (e.key === 'Enter') document.getElementById('token-submit').click(); });
async function loadStats() {
  ...
  } catch (err) { alert('Invalid token or error loading stats'); window.location.href = '/admin'; }
}
async function loadVerifications() {
  const tb = document.getElementById('table-body');
  tb.innerHTML = '<div class="empty"><span class="spinner"></span> Loading...</div>';
  try {
    const res = await fetch('/admin/api/verifications?token=' + encodeURIComponent(TOKEN) + '&limit=100');
    if (!res.ok) throw new Error('Unauthorized');
    const data = await res.json();
    const rows = data.verifications || [];
    if (rows.length === 0) { tb.innerHTML = '<div class="empty">No verifications yet.</div>'; return; }
    const html = ['<table><thead><tr><th>Time</th><th>Score</th><th>Verdict</th><th>IP</th><th>Fingerprint</th><th>Reasons</th></tr></thead><tbody>',
      ...rows.map(r => {
        const time = new Date(r.created_at).toLocaleString();
        const reasons = (Array.isArray(r.reasons) ? r.reasons : JSON.parse(r.reasons || '[]')).map(x => '• ' + x).join('<br>');
        return \`<tr><td class="mono">\${time}</td><td class="score">\${r.score}</td><td><span class="verdict verdict-\${r.verdict}">\${r.verdict}</span></td><td class="mono">\${r.ip || '—'}</td><td class="mono">\${r.fingerprint ? r.fingerprint.substring(0, 12) + '…' : '—'}</td><td class="reasons">\${reasons || '—'}</td></tr>\`;
      }), '</tbody></table>'].join('');
    tb.innerHTML = html;
  } catch (err) { tb.innerHTML = '<div class="empty">Error loading. Token may be invalid.</div>'; }
}
document.getElementById('refresh')?.addEventListener('click', () => { loadStats(); loadVerifications(); });
if (TOKEN) { loadStats(); loadVerifications(); setInterval(() => { loadStats(); loadVerifications(); }, 15000); }
</script>
</body>
</html>`;
}
